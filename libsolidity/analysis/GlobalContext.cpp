...
int magicVariableToID(std::string const& _name)
{
	...
	else if (_name == "sysblockhash") return -50;
	else
...
inline vector<shared_ptr<MagicVariableDeclaration const>> constructMagicVariables()
{
	...
	return {
		...
		magicVarDecl("sysblockhash", TypeProvider::function(strings{"uint256"}, strings{"bytes32"}, FunctionType::Kind::SYSBlockHash, false, StateMutability::View)),
